...
from PySide6.QtGui import QPixmap, QIcon, QImageReader, QFontMetrics, QColor, QImage
import sys
import os
import json
import numpy as np
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self._current_path = None            # currently active image path
        self._current_pixmap = None          # preview pixmap currently shown
        self._preview_base_image = None      # QImage preview base for current image
        ...
        self._image_parents = {}             # path -> QTreeWidgetItem parent
        self._edits = {}                     # still used for .lrc export
        ...
        # ---------- Left dock: Edit tree ----------
        lSidebar_dock = QDockWidget("Active Edits", self)
    ...
    def _on_saturation_changed(self, value: int):
        ...
        # Realtime preview ONLY for the big image
    ...
    def _on_edit_committed(self):
        ...
        # Save the current combined params for this image
        self._image_params[self._current_path] = dict(self._current_params)

        # Update the left tree (parent + Original + Saturation only)
    ...
    def _update_history_for_current_image(self):
        """
        Ensure the left tree has:
        - one parent per image path
        - under it: Original + single-effect children (Saturation only, etc.)
        """
        path = self._current_path
        if not path or self._current_pixmap is None:
            return

        # create parent if this is the first edit for this image
        if path not in self._image_parents:
            ...
            parent.takeChildren()  # clear old children
        ...
        # parent represents "combined" current edit
        ...
        # child 2: Saturation-only view (if param present)
        if "saturation" in combined_params:
            sval = combined_params["saturation"]
            child_sat = QTreeWidgetItem(parent)
            child_sat.setText(0, f"Saturation Mask: {sval}")
            child_sat.setIcon(0, edited_icon)
            child_sat.setData(
                0,
                Qt.UserRole,
                {"path": path, "params": {"saturation": sval}},
            )
    ...
    def _on_history_item_clicked(self, item: QTreeWidgetItem, column: int):
        ...
        # show that version; this also syncs sliders
    ...
    def _apply_styles(self):
        ...
        /* Central image area with top/bottom separators */
        #imageDisplay {{
    ...
    def _ensure_image_cached(self, path: str):
        ...
        # Smaller preview -> fewer pixels to process
        max_w, max_h = 1200, 800   # you can tweak this
        preview = img.scaled(
            max_w, max_h,
            Qt.KeepAspectRatio,
    ...
    def _apply_edit_params_to_current_image(self, params: dict):
        """Apply params to current preview base image and update display (NumPy-optimized)."""
        if self._preview_base_image is None:
            return

        # Work on a copy and ensure a known 4-byte format
        img = self._preview_base_image.copy()
        img = img.convertToFormat(QImage.Format_RGBA8888)

        w = img.width()
        h = img.height()
        bpl = img.bytesPerLine()  # bytes per line (may include padding)

        ptr = img.bits()          # PySide6: this is a memoryview
        # NO ptr.setsize(...) here – not needed and not supported

        # Build [h, stride_pixels, 4] view over the buffer
        arr = np.frombuffer(ptr, dtype=np.uint8).reshape((h, bpl // 4, 4))

        # Only first w pixels are actual image width
        rgb = arr[:, :w, :3].astype(np.float32)

        # Saturation: 128 = neutral, 0 = grayscale, 255 ~ 2x saturation
        sat_val = params.get("saturation", 128)
        factor = 1.0 if sat_val == 128 else sat_val / 128.0
        ...
        new_rgb = lum + (rgb - lum) * factor
        new_rgb = np.clip(new_rgb, 0, 255).astype(np.uint8)

        # Write back into the QImage buffer
        arr[:, :w, :3] = new_rgb
    ...
    def _on_thumbnail_clicked(self, item: QListWidgetItem):
        ...
        params = self._image_params.get(path, {})  # if never edited -> no params
        self._show_image_version(path, params, update_sliders=True)
    ...
    # ---------- Export / import (unchanged logic for now) ----------
        ...
        for abs_path, params in self._edits.items():
            try:
                if os.path.commonpath(
                    [os.path.abspath(abs_path), os.path.abspath(folder)]
